...
def compute_matrix_coloring(graph, sdp_type, verbose):
    ...
    Args:
        ...
        sdp_type (string): Non-strict, Strict, Strong vector coloring.
...
# TODO: 'iteration' param is bad, should not be used
...
def compute_directed_matrix_coloring(graph, beta_factors, sdp_type, verbose, is_alpha_constrained):
    result = None
    alpha_opt = None
    ...
    with Model() as Mdl:
        ...
        # n zmiennych beta_i
        betas = Mdl.variable(n, Domain.inRange(-1, 1))  # or lessThan(0.)

        # Constraints
        # TODO: niech ten ustalony wektor bedzie ostatni
        # na etapie matrix coloring nie ustalasz jeszcze polozenia tego wektora w przestrzeni
        # dopiero po dokonaniu Cholesky decomposition bedziesz mogl obrocic uklad tak aby znalazl sie w [1,0,...]
        ...
        # Dummy vector constraints
        # dla i=0,n-1 m{i,j} <= beta_i
        for i in range(n):
            Mdl.constraint(Expr.sub(m.index(n, i), betas.index(i)), Domain.lessThan(0.))
        ...
        alpha_opt = alpha.level()[0]
        level = m.level()
        result = [[level[j * (n + 1) + i] for i in range(n + 1)] for j in range(n + 1)]
        result = np.array(result)

        logging.info('Found directed matrix {0}-coloring'.format(1 - 1 / alpha_opt))
